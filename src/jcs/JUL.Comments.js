...
jul.apply(jul.get('JUL.Comments'), /** @lends JUL.Comments */ {
  ...
  filterTab: function (oEvent) {
    if (oEvent.keyIdentifier === 'U+0009') {
      ...
      try {
        ...
        this.setAttribute('value', sValue.slice(0, nStart) + JUL.Comments.project.coder._tabString + sValue.substr(nEnd));
        var l = JUL.Comments.project.coder._tabString.length;
  ...
  getComment: function (aFields) {
    ...
    for (var i = 0; i < aFields.length; i++) {
      ...
      aComments.push(this.config.linePrefix + this.template(oConfig.template, oField).replace(/\s+$/, '')
        .replace(/\n\r?/g, '\n' + this.config.linePrefix) + '\n');
    }
    if (!aComments.length) { return ''; }
    return (this.config.blockStart + aComments.join('') + this.config.blockEnd)
      .replace(/\t/g, this.project.coder._tabString).replace(/\n\r?/g, this.project.coder._newlineString);
  },
  /**
  ...
  */
  onInit: function () {
    ...
    var oThis = this;
    ample.getElementById('dialog-edit').addEventListener('windowshown', function() {
      oThis.focusText('textbox-edit', this);
    });
  },
  /**
  ...
  */
  wrapExport: function (sCode, bComment) {
    var sWrap = "(function(global, module) {\n'use strict';\n" +
    ...
    return sWrap.replace(/\t/g, this.project.coder._tabString).replace(/\n/g, this.project.coder._newlineString)
      .split('{code}', 2).join(sCode);
  },
  /**
