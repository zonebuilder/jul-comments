...
JUL.apply(JUL.Comments, /** @lends JUL.Comments */ {
	/**
	...
	*/
	createRows: function(sType, oInit) {
		...
		while (oRows.lastChild) { oRows.removeChild(oRows.lastChild); }
		sType = sType || 'project';
		...
		for (var sItem in oFields) {
			if (oFields.hasOwnProperty(sItem)) {
				...
				var oConfig = {tag: 'row', children: [
					...
					{tag: 'textbox', id: sId, style: 'width:500px'}
				]};
				if (oField.type === 'bool') {
				...
				if (oField.type === 'edit') {
					oConfig.height = oField.large ? 250 : 105;
					oConfig.children[1].multiline = true;
					oConfig.children[1].style = 'width:500px;height:' + (oField.large ? 250 : 105) + 'px';
				}
			...
			if (oControls.hasOwnProperty(sItem)) {
				...
				ample.getElementById(oControls[sItem]).setAttribute(oField.type === 'bool' ? 'checked' : 'value', typeof oInit[sItem] !== 'undefined' ?
					(JUL.typeOf(oInit[sItem]) === 'Array' ? oInit[sItem].join(', ') : oInit[sItem]) :
						(typeof oField.init === 'undefined' ?  (oField.type === 'bool' ? false : '') : this.config.projectFields[sItem].init));
			}
		}
	},
	/**
	...
	*/
	onInit: function() {
		...
		this.project.coder._jsonPrefixes = {
			func: '*=func:',
			regex: '*=regex:',
			newop: '*=newop:'
		};
		...
		for (var sItem in this.config.commentTemplates) {
