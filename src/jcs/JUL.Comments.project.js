...
JUL.apply(JUL.Comments.project, /** @lends JUL.Comments.project */ {
	...
	*/
	emptyTabs: function() {
		...
		while (oTabs.lastChild) { oTabs.removeChild(oTabs.lastChild); }
	},
	/**
	...
	*/
	getCode: function(sPath) {
		...
		sText = sText + this.coder._newlineString + this.app.getComment((this.current.comments[this.state._rootPath] || []).concat(this.current.useJul ? {field: 'name', value: sPath} : []) );
		var bWrap = this.current.useJul && JUL.typeOf(this.ref(sPath)) === 'Object';
		var sText1 = (bWrap ? this.coder._newlineString + 'JUL.apply(' : '') + fName(sPath) + (bWrap ? ', /** @lends ' + sPath + ' */ ' : ' = ');
		...
		if (this.current.useJul) { sText = sText + "JUL.ns('" + sPath.replace(/'/g, "\\'") + "');" + this.coder._newlineString; }
		for (j = 0; j < this.current.expandProto.length; j++) {
			...
			if (this.inside(sProto, sPath) && aExclude.concat(aPut).indexOf(sProto) < 0) {
				...
				sText1 = sText1 + (this.current.useJul ? this.coder._newlineString + 'JUL.apply(' : '') + fName(sProto) + '.prototype' +
					(this.current.useJul ? ', /** @lends ' + this.state._rootPath + ' */ ' : ' = ');
				oData = this.parse(this.get(this.state._rootPath), this.state._rootPath, null, null, aExclude.concat(aPut));
				sText1 = sText1 + this.coder.obj2str(oData, false, this.decorator) + (this.current.useJul ? ');' : ';') + this.coder._newlineString;
			}
		}
		var sText2 = '';
		for (i = 0; i < aPut.length; i++) {
			...
			for (j = 0; j < this.current.expandProto.length; j++) {
				...
				if (this.inside(sProto, aPut[i]) && aExclude.indexOf(sProto) < 0) {
					this.state._rootPath = sProto + '.prototype';
					sText2 = sText2 + this.coder._newlineString + this.app.getComment(this.current.comments[this.state._rootPath] || []);
					sText2 = sText2 + (this.current.useJul ? this.coder._newlineString + 'JUL.apply(' : '') + fName(sProto) + '.prototype'  +
						(this.current.useJul ? ', /** @lends ' + this.state._rootPath + ' */ ' : ' = ');
					oData = this.parse(this.get(this.state._rootPath), this.state._rootPath, null, null, aExclude);
					sText2 = sText2 + this.coder.obj2str(oData, false, this.decorator) + (this.current.useJul ? ');' : ';') + this.coder._newlineString;
				}
			}
		}
		...
		return this.app.template(sTemplate, {
			code: sText + (this.current.useJul ? sText2 + sText1 : sText1 + sText2),
	...
	*/
	getId: function(sPath, sPrefix) {
		return (sPrefix || '') + sPath.replace(/\\./g, '-----').replace(/\./g, '----').toLowerCase();
	},
	/**
	...
	getView: function(sPath) {
		...
		if (aPath.length > 1) {
			...
			if (JUL.typeOf(oParent) === 'Object') {
				...
				sCode = sCode + JUL.trim(this.coder.obj2str(oKey).slice(1, -1)).slice(0, -1);
			}
		}
	...
	*/
	onSelectNode: function(oNode) {
		...
		if (oNode) {
			...
			this.setPreview(sPath, false, oGet);
			this.selectTab(sPath);
		}
		else {
			this.setChecks();
			...
			for (var sItem in oData) {
				if (oData.hasOwnProperty(sItem) && aPut.indexOf(sItem) < 0) {
					if (this.current.sortKeys.indexOf(sPath) > -1) {
						try {
						if (typeof oData[sItem] === 'function' ||
							(typeof oData[sItem] === 'string' && this.coder._regExps.functionStart.test(oData[sItem]))) {
	...
	*/
	removeField: function() {
		...
		while (oList.selectedItems.length) {
			...
			var sId = this.app.getWhere(oLast);
	...
	*/
	removeTree: function(oNode, bOnlyChildren) {
		if (!oNode) { oNode = ample.getElementById('tree-dom').body; }
		var oChildren = oNode.children;
		if (oChildren) {
			while (oChildren.lastChild) { this.removeTree(oChildren.lastChild); }
		}
		if (!bOnlyChildren && oNode.nodeName === 'xul:treeitem') { oNode.parentNode.removeChild(oNode); }
	},
	/**
