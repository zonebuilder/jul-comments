...
jul.apply(jul.get('JUL.Comments.project'), /** @lends JUL.Comments.project */ {
  /**
  ...
  */
  coderConfig: {
    _useJsonize: false, includeProperty: '_$inc', referencePrefix: '_@ref:', _usePrefixes: true, _tabString: '  '
  },
  /**
  ...
  */
  applyComment: function () {
    ...
    ample.getElementById('button-apply').setAttribute('label', 'Apply');
  ...
  */
  buildTree: function (sPath, nLevel) {
    ...
    var sLabel = sId.pop().replace(/:{5}/g, '.').replace(/\\(\[|\])/g, '$1');
  ...
  */
  ...
  /**
    Transforms the given code into a commented one. It acts as a callback in JUL.UI.obj2str().
  ...
  */
  ...
  if (!sId) { sId = 'listbox-comment'; }
    ...
    while (oList.body.lastChild) {
      sId = oList.body.lastChild.getAttribute('id');
      JUL.Comments.empty(oList.body.lastChild, true);
      if (sId) { delete this.app.state.map[sId]; }
    }
  },
  /**
    Removes all tabs from the code tab box
  */
  emptyTabs: function () {
    ...
    while (oTabs.lastChild) {
      ...
      JUL.Comments.empty(oTabs.lastChild, true);
      delete this.app.state.map[sId];
      delete this.state.revMap['tab-' + sPath];
    }
    oTabs.parentNode.selectedTab = null;
    oTabs.parentNode.selectedIndex = -1;
  },
  /**
  ...
  */
  fillCode: function (sPath, bUpdate) {
        ...
        this.app.focusText('textbox-code', null);
  ...
  */
  findObject: function (sGet) {
    if (!JUL.UI._regExps.variable.test(sGet.split('.', 2)[0])) {
      window.alert('Invalid namespace base');
      return;
    }
    var oGet;
    try {
    ...
    if (!this.current.files[sGet]) {
      this.current.files[sGet] = {
        ...
        template: this.app.config.fileFields.template.init.replace('{code}',
          this.current.useJul ? this.app.wrapExport('{code}', true) : '{code}')
      };
    }
  ...
  */
  ...
  /**
    Returns a member of the saved JSON object
  ...
  */
  getCode: function (sPath) {
      var fName = function(sName) {
        ...
        if (aName.length) {
          aName = aName.map(function(sItem) {
            sItem = sItem.replace(/:{5}/g, '.').replace(/\\(\[|\])/g, '$1');
      ...
      var sText1 = (bJul ? this.coder._newlineString + (bWrap ? 'jul.apply(' :  'jul.ns(') : '') +
        (bJul ? (bWrap ? 'jul.get(' : '') + "'" + sPath.replace(/'/g, "\\'") + "'" + (bWrap ? ')' : '') : fName(sPath)) +
        (bJul ? ', /** @lends ' + fName(sPath) + ' */ ' : ' = ');
      ...
      for (j = 0; j < this.current.expandProto.length; j++) {
        ...
        if (this.inside(sProto, sPath) && aExclude.concat(aPut).indexOf(sProto) < 0) {
          ...
          sText1 = sText1 + (bJul ? this.coder._newlineString + 'jul.apply(' : '') +
            (bJul ? "jul.get('" + sProto.replace(/'/g, "\\'") + "')" : fName(sProto)) + '.prototype' +
      ...
      for (i = 0; i < aPut.length; i++) {
        ...
        sText2 = sText2 + this.coder._newlineString + this.app.getComment((this.current.comments[this.state._rootPath] || []).concat(bJul ? {field: 'name', value: fName(aPut[i])} : []));
        sText2 = sText2 + (bJul ? "jul.ns('" + aPut[i].replace(/'/g, "\\'") + "', " : fName(aPut[i]) + ' = ');
        ...
        for (j = 0; j < this.current.expandProto.length; j++) {
          ...
          if (this.inside(sProto, aPut[i]) && aExclude.indexOf(sProto) < 0) {
            ...
            sText2 = sText2 + (bJul ? this.coder._newlineString + 'jul.apply(' : '') +
              (bJul ? "jul.get('" + sProto.replace(/'/g, "\\'") + "')" : fName(sProto)) + '.prototype'  +
              (bJul ? ', /** @lends ' + fName(this.state._rootPath) + ' */ ' : ' = ');
      ...
      return this.app.template(sTemplate, {
        ...
        .replace(/\t/g, this.coder._tabString).replace(/\n\r?/g, this.coder._newlineString)
        .split('{:-g_code-:}', 2).join(sText + (bJul ? sText2 + sText1 : sText1 + sText2));
  },
  /**
  ...
  */
  getView: function (sPath) {
    ...
    if (aPath.length > 1) {
      ...
      var sEnd = aPath.pop().replace(/\\\./g, '.').replace(/\\(\[|\])/g, '$1');
  ...
  */
  ...
  /**
    Fires when the user checks 'expand prototype'
  ...
  */
  onGenerateComments: function () {
    ...
    for (var i = 0; i < oItems.length; i++) {
      ...
      var oGet = this.ref(sPath);
      var sName = sPath.replace(/\\\./g, ':::::').split('.').pop().replace(/:{5}/, '.').replace(/\\(\[|\])/g, '$1');
  ...
  */
  onSelectNode: function (oNode) {
    ...
    if (oNode) {
      ...
      if (this.state.lastChanged && this.state.lastChanged.path === sPath) {
        ample.getElementById('button-apply').setAttribute('class', 'apply');
        ample.getElementById('button-apply').setAttribute('label', 'Apply*');
      }
      else {
    ...
    case 'Object':
      ...
      for (i = 0; i < aKeys.length; i++) {
        ...
        if (oData.hasOwnProperty(sItem)) {
           try {
           oReturn[sItem] = this.parse(oData[sItem], sPath + '.' + sItem.replace(/\./g, '\\.').replace(/(\[|\])/g, '\\$1'), fAction, oContext, aExclude, nLevel + 1);
           }
           catch (e2) {}
        }
      }
  ...
  */
  removeField: function () {
    ...
    while (oList.selectedItems.length) {
      ...
      JUL.Comments.empty(oLast, true);
      delete this.app.state.map[sId];
    }
    this.onFieldChange();
  },
  /**
    Removes a (sub)tree from the DOM tree pane
  ...
  */
  removeTree: function (oNode, bOnlyChildren, bSelf) {
    ...
    if (!bSelf) {
      ...
      if (!bOnlyChildren && oParent.nodeName === 'xul:treechildren' && !oParent.items.length) {
        ...
        JUL.Comments.empty(oParent.children, true);
  ...
  /**
    Resets the comment fields list box to the stored comment
  */
  ...
  */
  setPreview: function (sPath, bNew, oGet) {
    ...
    if (bNew) {
      ample.getElementById('description-preview').setAttribute('tooltiptext', 'Comment not saved. Press \'Apply\' to save it.');
    }
    else {
    ...
    this.app.focusText('textbox-preview', null);
  },
  /**
