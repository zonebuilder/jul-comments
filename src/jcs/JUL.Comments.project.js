	...
	coderConfig: {
		_useJsonize: false, _usePrefixes: true,
		 _jsonReplacer: function(sKey, oValue) {
			JUL.UI._this_ = this;
			var sOut = JUL.UI._jsonReplacer(sKey, oValue);
	...
	*/
	getView: function(sPath) {
		...
		if (oGet && typeof oGet === 'object' && typeof oGet.toJSON === 'function') {
			sCode = sCode + this.coder.obj2str(this.coder._jsonReplacer('', oGet.toJSON())) + '\n';
		}
		else {
			switch (sType) {
			...
			case 'String':
			case 'Undefined':
				sCode = sCode + this.coder.obj2str(this.coder._jsonReplacer('', oGet)) + '\n';
			break;
